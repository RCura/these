...
\section{Capter les sorties de SimFeodal}\label{sec:capter-sorties-simfeodal}
...
\clearpage
\section{Comment explorer les données de SimFeodal ?}\label{sec:explorer-sorties-simfeodal}
...
Le choix d'une méthode d'organisation des données en sortie de simulations ne relève donc pas ici d'une quelconque coquetterie technique, mais, au contraire, conditionne et contraint fortement aussi bien les modalités de création des indicateurs que les possibilités de la plate-forme d'exploration de les afficher de manière interactive.
	...
	Une fois le SGBD destiné à stocker les données en sortie de SimFeodal, encore faut-il choisir la manière de les organiser. \hl{Ne pas oublier de rédiger cette courte intro}
...
\clearpage	
		...
		\begin{figure}[H]
			...
			(2) - Les simulations associées sont isolées\\
			(3) - Répercussion de ce filtre dans la table des simulations\\
			(4) - L'ensemble des données de SimFeodal sont filtrées en répercussion}
			\label{fig:MCD_SimEDB_etapes}
		\end{figure}
	
	\paragraph*{Conclusion}
	\hl{RÉDIGER CONCLUSION DE LA PARTIE DONNÉES ICI !}
