...
\todobox{Intro : bien indiquer le fait que cette dernière dimension est bien plus qualitative et subjective que les autres. Par exemple, étude des cartes (pas de réplication donc). Ouverture en expliquant qu'on touche dans cette dimension bien plus à une étude des dynamiques internes, au niveau de l'agent, qu'à un niveau réellement agrégé.
\begin{itemize}
	\item Pas trop de déplacements des FP
	\item Privilégier déplacements locaux
	\item Régularité du maillage paroissial
\end{itemize}}
...
\begin{mdframed}[backgroundcolor=gray!10,footnoteinside=false]
	(\cref{fig:nb-deplacements-v0} et \cref{fig:type-deplacements-v0}).
\end{mdframed}
...
Impact directemnt la fixation : quand FP trop mécontents, ne se fixent jamais à un endroit et l'apparente stabilité des agrégats n'est due qu'à un effet pendulaire qui se compense à peu près.

\paragraph{Dispersion}
Voir \autoref{par:polarisation-dispersion}, p. \pageref{par:polarisation-dispersion} : 


\paragraph{Dispersion des FP/agrégats}
