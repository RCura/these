...
\begin{figure}[H]
	...
	\caption{Les étapes du mécanisme de détection et de mise à jour des agrégats.}
...
On exécute un algorithme de classification basé sur la distance, proche de DBSCAN : sont considérés comme agrégats les \textit{clusters} constitués d'au moins 5 foyers paysans espacés de moins de 100~m (\textbf{B}).
On dessine alors la géométrie des agrégats, sous la forme de l'enveloppe convexe des foyers paysans qui les composent (\textbf{C}).
Les foyers paysans situés dans la surface d'un agrégat y sont ajoutés (\textbf{D}).
On crée ensuite des \textit{buffers} de 100~m autour des agrégats et on y rattache à nouveau les foyers paysans présents dans la surface (\textbf{E}).
Dernière étape dans la détection des agrégats, et afin de ne pas multiplier les agrégats proches les uns des autres, on procède à une étape de fusion : sont fusionnés les agrégats qui s'intersectent (\textbf{F}).\\
Les trois dernières étapes concernent l'identification des propriétés des agrégats, et en particulier de leur affectation d'une communauté.
...
Le nouvel agrégat récupère alors les propriétés de l'ancien agrégat dont la superficie d 'intersection est la plus importante, c'est-à-dire l'agrégat vert ici (\textbf{I}).
