...
Le chapitre précédent (\hl{ref chap 1}) visait à présenter le positionnement de cette thèse, située entre géographie, modélisation et informatique.
...
Dans ce chapitre, nous présentons donc de manière détaillée le modèle de simulation auquel notre groupe de travail a abouti : SimFeodal, acronyme de \og \textbf{Sim}ulation de la \textbf{F}ixation, de l'\textbf{É}mergence et de l'\textbf{O}rganisation \textbf{D}ynamique d'\textbf{A}grégats de population \textbf{L}ocalisés\fg{}.
...
Ces choix ancrent SimFeodal dans une approche \og KIDS\fg{} \autocite{edmonds_kiss_2005}, caractérisée par une modélisation initialement détaillée, que l'on cherche à rendre plus parcimonieuse dans un second temps (\hl{voir chapitre 1}).
Ce type de modélisation permet de concilier différentes formes de procéder dans l'élaboration de modèles, en épargnant en particulier aux experts thématiciens de mener des généralisations hâtives, sans ancrages empirique, qui ne peuvent que nuire à la qualité du modèle, tant du point de vue méthodologique que thématique.
...
Comme indiqué dans le \hl{chapitre 1}, les experts thématiciens sont chacun spécialistes de tel ou tel aspect du modèle (lignages seigneuriaux, communautés, etc.), aspects qui seront dès lors modélisés de manière plus fine et détaillée.
SimFeodal a de plus été construit de manière itérative et incrémentale (\hl{ref encadré}), ce qui renforce cette hétérogénéité de niveaux de généralisation.
...
On présente, dans ce chapitre, la version \og 6.3\fg{} du modèle\footnote{
...
Le contexte de réalisation du modèle SimFeodal a été présenté dans le \hl{chapitre 1, partie contexte TransMonDyn}, de même qu'une première description, brève, des processus sociaux et spatiaux que l'on cherche à reproduire dans le modèle (\hl{chapitre 1, \og Identifier des régimes et transitions\fg{}}).
...
\begin{center}
	\hl{Manque bout de commentaire de Julie dans le scan (bas droite) : à traiter plus tard.}
\end{center}

Le contexte historique et historiographique dans lequel s'inscrit le modèle SimFeodal est riche et très étudié par les médiévistes.
...
Le choix du carré s'inscrit dans la volonté d'isotropie, et l'espace ainsi défini est proche de l'ordre de grandeur des diocèses médiévaux, maillage territorial relativement stable dans le temps depuis cette époque médiévale jusqu'à aujourd'hui.
...
La superficie du département d'Indre-et-Loire actuel, ou encore du diocèse de Tours qui est à peu près équivalent à ce département, sont ainsi d'environ 6 000 km².
...
Prolonger cette date d'observation des résultats du modèle permet d'analyser le comportement du modèle après la période même de la transition identifiée, à une date où le système de peuplement est pleinement inscrit dans son régime post-transition (régime 2 -cf. \hl{chap1, contexte, transitions et régimes}).
...
La mise en place de mécanismes tributaires de dates nous permet ainsi de représenter des événements exogènes (voir \hl{chap 1}) au modèle, qui peuvent dès lors servir de déclencheurs ou de catalyseurs à des processus de longue durée.
...
Il est ainsi difficile de s'abstraire du contexte spatial, en particulier dans un modèle mettant en place des migrations.
...
\begin{center}
	\hl{Manque prise en compte commentaire Julie en bas de page, coupé dans scan.}
\end{center}	
...
\begin{tcolorbox}[breakable,left=0pt,right=0pt,top=0pt,bottom=0pt,
	...
	Les diagrammes de Voronoï sont fortement sensibles à l'ajout où à la suppression de points.
	Il est donc nécessaire, à chaque pas de temps, de ré-initialiser la construction du maillage, c'est-à-dire de répéter l'opération de création du diagramme de Voronoï, en repartant d'une situation \og neutre\fg{} d'où l'on aura supprimé le maillage pré-existant\footnotemark.
...
\end{tcolorbox}
	...
	\item \textbf{Promotion de paroisses \og rurales\fg{}} Tout au long de son développement et à mesure de l'importance sociale qu'il acquière, on sait que le maillage paroissial se densifie, pour parvenir en fin de période au maillage quasi-communal qui en est hérité aujourd'hui.
...
Thématiquement, l'hypothèse émise pour expliquer ces migrations est qu'elles résultent de l'insatisfaction, pour les foyers paysans, de leurs principaux besoins, qu'ils soient d'ordre matériel, spirituel ou en termes d'intégrité physique.
...
Au fur et mesure de l'avancement de la période, les travaux empiriques montrent que le pouvoir central s'efface et que les ressorts locaux subissent un émiettement important.
...
Les seigneurs prélevaient des droits auprès de leurs sujets pour différentes raisons : droits de haute et moyenne justice -- taxes universelles dont chacun devait s'acquitter --, mais aussi droits d'usages, banaux par exemple, autour de l'utilisation de tel ou tel équipement collectif (le four à pain banal, le moulin, etc.).
...
\begin{center}
	\hl{Ajouter refs via mail de Julie}
\end{center}
...
L'enregistrement des données d'un modèle est un problème complexe, dont les enjeux et difficultés sont discutés dans le chapitre 5 (\hl{section 5.1}).
...
Pour chacun de ces 10 concepts (voir \cref{tab:proto-ODD}
%\footnote{
...
%}
), nous décrivons brièvement, le cas échéant, comment ils sont appliqués dans SimFeodal.

\paragraph{Émergence} Ce mécanisme constitue l'un des fondements de nombreux modèles complexes, dont SimFeodal.
La diversité des éléments analysés dans le modèle est trop importante pour établir une liste des éléments qui émergent (\hl{voir le chapitre 3, section 3.? par exemple}).
...
On ne peut prédire le comportement du modèle, ni même l'effet de certains paramètres, sans le simuler (\hl{voir le chapitre 6, section analyse de sensibilité, ou chapitre 3 sur l'évaluation des params}).
...
Pour représenter celles qui sont dotées des droits paroissiaux (environ une cinquantaine au début de la période), on procède à un tirage aléatoire d'un tiers\footref{ftn:nombre-parametrable} de ces églises auxquelles sont attribués ces droits.
...
Les foyers paysans suivants (une trentaine\footref{ftn:nombre-parametrable}) sont localisés, de manière aléatoire, à l'intérieur d'un certain rayon du premier (voir \cref{meca-agregats} et \cref{fig:init-fp}).

On répète cette démarche pour les \og villages\fg{}, en plaçant à nouveau quelques\footref{ftn:nombre-parametrable} agrégats dans l'espace, cette fois constitués d'une dizaine\footref{ftn:nombre-parametrable} de foyers paysans.
...
L'initialisation spatiale des seigneurs ne concerne donc que les petits seigneurs, dont on estime empiriquement le nombre initial à une vingtaine\footref{ftn:nombre-parametrable}.
...
Quand bien même ce n'est pas le sens traditionnel d'\textit{inputs}, la multiplicité des paramètres et leur forte diversité remplit le même rôle (\hl{voir chapitre 4}).
...
\end{figure}
\begin{center}
\hl{Dans étape E, pas sûr que les 2 buffers soient de la même couleur, à vérifier.}
\end{center}
...
\begin{center}
	\hl{Reprendre la conclusion cf. remarques Julie}
\end{center}
...
En dépit de cette forte hétérogénéité dans les mécanismes, il nous semble toutefois important de pointer aussi la certaine généricité qui en caractérise les fondations.
