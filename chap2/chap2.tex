...
\begin{itemize}
	...
	\item 29/10/2019 : fin reprises : envoi en relecture à Thomas \& Julie
\end{itemize}
...
Au delà de la combinaison de démarches incrémentales et itératives\footnote{
...
Dans cette partie, il nous semble important de remettre en avant l'aspect thématique du modèle, c'est-à-dire aussi bien les questions thématiques qu'il cherche à adresser que le contexte historiographique dans lequel ces questions s'inscrivent.

\subsection{Contexte historiographique \label{subsec:contexte-historio}}

Le contexte historique et historiographique qui accompagne le modèle SimFeodal est riche et très étudié par les médiévistes.
Il dessine les contours d'un paysage scientifique, entre histoire et archéologie, qui cherche à comprendre les origines du féodalisme et les changements que ce système a provoqué autour de l'\og{}An Mil\fg{}~\autocite{duby1967mil}\footnote{
	Pour ce contexte, nous avons préféré reprendre, en la citant, une partie de l'introduction d'un chapitre d'ouvrage collectif \autocite{cura_transition_2017}, rédigé par les auteurs du modèle et donc validé par chacun d'entre eux.
	Cela nous semble préférable à une certaine forme d'\og{}auto-paraphrase\fg{} qui ne reviendrait qu'au travestissement d'un texte porté avant tout par l'expertise empirique des archéologues et historiens qui en sont les co-auteurs. 
}.
...
Dans ce contexte, nous avons cherché à étudier, avec une approche \og résolument géographique\fg{}~\autocite[302]{cura_transition_2017}, les causes et conditions des processus spatiaux, à l'œuvre durant cette période, de polarisation, de fixation, et de hiérarchisation du système de peuplement rural.
...
Le \og monde\fg{} simulé n'est pas un espace géographique précis qui correspondrait à l'observation empirique.
...
Pour représenter, parmi ces édifices religieux, ceux qui sont dotés des droits paroissiaux (environ une cinquantaine au début de la période), on procède à un tirage aléatoire d'un tiers\footref{ftn:nombre-parametrable} de ces églises auxquelles sont attribués des droits paroissiaux.
...
Les foyers paysans suivants (une trentaine\footref{ftn:nombre-parametrable}) sont localisés, de manière aléatoire, à l'intérieur d'un certain rayon (voir \cref{meca-agregats}) du premier (\cref{fig:init-fp}).
...
On en a déjà mentionné les raisons dans l'introduction : les niveaux de précision des connaissances des différents objets modélisés sont très variables, et très liés aux parcours et sujets de recherche individuels des thématiciens impliqués.
