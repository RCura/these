...
\hl{Regarder dans :} \textcite[1590-1591]{epstein2006remarks}
...
%		
%		\item[Sorties] Les sorties d'un modèle représentent ce qui est produit par le modèle durant la simulation, et enregistré en fin de simulation.
...
%Nous en proposons donc une troisième, que nous avons choisi de nommer \og paramètres commensurables\fg{}\footnote{
...
%Pour différents modélisateurs, un même paramètre pourra donc être considéré comme relevant du contexte ou des mécanismes, ou encore être  ou commensurable, ou encore comme commensurable ou technique.
%\todobox{
...
%	\begin{TAB}(b,3cm,3cm)[11pt]{|c|c|c|}{|c|c|c|}% (rows,min,max)[tabcolsep]{columns}{rows}
%		\thead{\diagbox{Par}{Paramètres}} & \thead{Endogène}  & \thead{Exogène}  \\
%		
%		\thead{Technique} & \makecell{Paramètre technique} & \makecell{---\\ (ou constante?)} \\
%		
%		\thead{Thématique} & \makecell{Paramètre\\thématique\\endogène\\(ex commensurable)} & \makecell{Paramètre\\thématique\\exogène\\(ex empirique)}
%		
%	\end{TAB}
%\end{encadre}
...
\begin{encadre}{Construction, paramétrage et calibrage}{termes-calibration}
	...
	\begin{description}[style=nextline]	
		...
		}, et le calibrage aboutit donc souvent à un ensemble de configurations possibles, constituant par exemple un optimum de Pareto (\fixref{Trouver ref}).
...
\pagebreak
\begin{footnotesize}
	\begin{longtable}{ m{.05\textwidth} m{.08\textwidth} m{.12\textwidth} m{.65\textwidth}}
