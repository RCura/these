...
%-- selon le nombre de foyers paysans contenus dans chaque agrégat\footnote{
...
%avec la création de nouvelles paroisses au sein des agrégats, on devrait voir apparaître de nombreuses paroisses se partageant un espace très réduit.
...
%On a choisi ici de discrétiser l'espace modélisé (de dimensions 80×80 km) en mailles de 4×4 km, pour un total de 400 carreaux de 16 km².
...
%Cette augmentation devrait toutefois être moins rapide que celle du nombre de paroisses, puisque les églises paroissiales créées au sein des agrégats devraient logiquement ne pas desservir de nouvelles mailles, mais plutôt se situer dans des mailles déjà occupées.
...
%Selon les mécanismes de SimFeodal, on s'attend à ce que l'indicateur ainsi produit augmente au cours du temps, à mesure que de nouvelles églises paroissiales viennent desservir le territoire, de manière régulière (comme l'évolution du nombre d'églises paroissiales).
...
\begin{quotation}
	\noindent \og 1. \textsc{math.} Quantité à fixer librement, maintenue constante, dont dépend une fonction de variables indépendantes, une équation ou une expression mathématique. --- Variable en fonction de laquelle on exprime chacune des variables d'une équation.\\
...
\begin{quotation}
	\noindent \og
	For the function $f(x)=ax^2+bx+c$, could we learn something by leaving the expression in terms of the symbols $a$, $b$, and $c$ and seeing how $f(x)$ depends on the parameters? Maybe we could fix $x=2$ and look how $f(2)$ changes as we let $a$ vary.
	
	\noindent If we do such manipulations and look at how the output of a function depends on varying a parameter, then we are treating the function as though the parameter were a input variable. But that's OK, as the difference between variables and parameters is really just a matter of perspective.
	\fg{}\\
...
\begin{quotation}
	\noindent \og \textit{In statistics}, the most common use of \og parameter\fg{} is for a characteristic of a population, or of a distribution of scores, described by a statistic such as a mean or a standard deviation. For example, the mean (average) score of on the midterm exam in Psychology 201 is a parameter. It describes the population composed of all those who took the exam.\fg{}\\
...
%\toChange{D'où une confusion fréquente entre paramètre et valeur initiale, en particulier dans le domaine de la simulation à base d'agents.}{Lena : vraiment ?}
...
\subsubsection{Les paramètres dans SimFeodal}
...
%%%%% Pas adapté  %%%%
%\textcite{gilbert_simulation_2005} restent très flous en utilisant les termes de \textit{checking} et de \textit{debugging}, qu'ils inscrivent dans une démarche plus large de \textit{verification} correspondant à la validation interne.
%\textcite{balci_validation_1994} crée et emploie l'acronyme \og \textit{VV\&T}\fg{}\footnote{\textit{Validation, Verification and Testing}}, à portée plus large puisqu'il s'applique à chacune des étapes de la modélisation
%%%%%  %%%%

Le plus souvent, une fois le modèle construit, le modélisateur s'attache à son \og calibrage\fg{}, en cherchant pour chaque paramètre la ou les valeurs qui permettront au modèle de s'approcher des données empiriques devant être reproduites, c'est-à-dire la conjugaison de \og valeurs optimales\fg{} de paramètres minimisant l'écart entre les données simulées et les données empiriques de contrôle.
...
Nous définissons dans l'\cref{enc:termes-calibration} le sens donné à ces termes dans le cadre de ce manuscrit.
\bigskip
...
\begin{figure}[H]
\includegraphics[width=\textwidth]{img/schema_definition_parametrage.pdf}
...
Le paramétrage du modèle est l'étape où l'on vérifie la cohérence empirique et simulée du modèle.
...
Ce modèle statistique
%\footnote{
%	Pour une description plus complète, se référer à l'ouvrage de \textcite{pumain_les_2001}.
%}
, dans la formulation qu'en a faite \textcite{stewart_demographic_1948}, vise à prédire des flux (démographiques, marchands etc.) potentiels entre des lieux à partir d'une analogie avec la loi physique de la gravitation.
...
\vspace*{-.75em}Ce modèle présente trois valeurs empiriques ($M_i$, $M_j$ et $d_{ij}$) et deux paramètres ($k$ et $\alpha$).
...
$\alpha$ est un paramètre commensurable pour les raisons explicités dans l'\cref{enc:types-parametres}.
...
Au début de la simulation, des agents de deux types, en proportions similaires, sont distribués aléatoirement dans l'espace du modèle. Chaque agent occupe une cellule, et le nombre total de cellules occupées (et donc d'agents) dépend d'un paramètre de densité $d$ qui représente la part (entre $0\%$ et $100\%$) du nombre de cellule qui sera occupée
%\footnote{
...
%\footnote{
%	C'est-à-dire manuelle, au contraire de méthodes automatiques plus récentes.
%}
...
\paragraph{Un ou des modèles à paramétrer ?} \label{sssec:parametrer-un-des-modeles}
...
Certains auteurs \autocite{} plaident pour une construction modulaire de modèles, chacun des mécanismes et complexifications devant être autonomes afin de pouvoir les assembler en autant de combinaisons que nécessaire à une exploration d'ensemble de leurs interactions.
...
\toChange{Cet exemple renforce le caractère nécessaire du paramétrage, et qui plus est, de la continuité de cette étape, qui ne peut être pensée que comme un calibrage finale d'un modèle abouti, auquel on ne peut en fait parvenir que par des paramétrages réguliers de modèles successifs moins aboutis.}{Lena: Pas clair et pas assez logique}
...
Décrire méthodo construction plot :
- on analyse chaque \textit{commit} du code = enregistrement (pour historicisation) des changements
- dans chaque commit, on observe le nombre de lignes de code de chacun des fichiers,  (du modèle uniquement, exclusion de la documentation et des tests) qui ont changé 
- on agrège par jour (parfois, plusieurs commits qui viennent corriger des erreurs ou des oublis du commit précédent)
- on dans cette agrégation, on calcule la taille médiane des changements (en lignes de codes modifiées) et la taille totale (somme des lignes modifiées)
- pour la représentation, on normalise (réduction sans centrage) pour que les sommes et médianes soient comparables (en fait : max médiane $\approx$ 300 lignes, max somme $\approx$ 1500 lignes)


\begin{figure}[H]
	\includegraphics[width=1.2\linewidth]{img/explo_edits_code_clean.pdf}
...
La \cref{fig:explo-edits-code} fait apparaître plusieurs types de changements :
phases successives de construction et d'évaluation, + feedback paramétrage
- phases assez nettes : plein de petites modifs (somme et médiane) précèdent les changements conséquents dans le modèle : 
- petites modifs : souvent des modifs successives de paramètres : médianes faibles = quelques lignes changées : calibrage
- modifs moyennes : changement des mécanismes : paramétrage sur mécanismes
- grosses modifs (médiane) : changement des sorties, création des expériences d'exploration + nettoyage des anciennes versions (très gros changements)

-> on retrouve les versions (exemple de 6.2 à 6.5) dans le rythme des modifications du code : chaque pic (en somme) = sous-version : 6.1, 6.2, 6.3, 6.4, 6.5


\paragraph{Avant propos sur le paramétrage d'un modèle complexe.}
...
Nous ne détaillerons pas ici les différentes phases de construction précédentes. Nous reviendrons toutefois sur plusieurs d'entre elles, en guise d'exemples illustratifs, dans le \hl{chapitre 7}.


\paragraph{Les étapes du paramètrage de SimFeodal}
