...
%	\item Janvier/Février 2019 : Corrections après commentaires de Lena sur 3.1 à 3.1.4
	...
	\item : TODO : intro + conclusion du chapitre, + micro-transitions dans chapitre
\end{itemize}
...
Dans ce modèle, on peut donc dénoter trois paramètres, $N$, $S$ et $d$.
...
Dans le paragraphe précédent, de la même manière que dans le \hl{chapitre 2}, nous avons plusieurs fois mentionné les versions et sous-versions de SimFeodal.
...
La \cref{fig:explo-edits-code} est une représentation de cette analyse, qui permet de visualiser le \og rythme\fg{} des changements de SimFeodal et d'en tirer quelques remarques.
...
\begin{figure}[H]
	...
	\caption[Temporalité du paramétrage du modèle.]{Une exploration visuelle du rythme des changements de SimFeodal.\\
		\textit{N.B. : la période \og B\&C\fg{} correspond à ces phases dans la \cref{fig:comits-periodes}.}}
...
\begin{itemize}
	\item phases successives de construction et d'évaluation, + feedback paramétrage
	\item phases assez nettes : plein de petites modifs (somme et médiane) précèdent les changements conséquents dans le modèle
	\item petites modifs : souvent des modifs successives de paramètres : médianes faibles = quelques lignes changées : calibrage
	\item modifs moyennes : changement des mécanismes : paramétrage sur mécanismes
	\item grosses modifs (médiane) : changement des sorties, création des expériences d'exploration + nettoyage des anciennes versions (très gros changements)
	\item -> on retrouve les versions (exemple de 6.2 à 6.5) dans le rythme des modifications du code : chaque pic (en somme) = sous-version : 6.1, 6.2, 6.3, 6.4, 6.5
\end{itemize}
